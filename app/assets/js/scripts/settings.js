        ...
        ele.onclick = async e => {
            ...
            if (isJavaExecSel && process.platform === 'win32') {
                options.filters = [
                    {
                        name: Lang.queryJS(
                            'settings.fileSelectors.executables'
                        ),
                        extensions: ['exe'],
                    },
                    {
            ...
            if (v.tagName === 'INPUT') {
                ...
                if (v.classList.contains('rangeSlider')) {
                    ...
                    if (cVal === 'MinRAM' || cVal === 'MaxRAM') {
                        ...
                        if (val.endsWith('M')) {
                            val =
                                Number(val.substring(0, val.length - 1)) / 1024;
                        } else {
            ...
            AuthManager.addMicrosoftAccount(authCode)
                ...
                .catch(displayableError => {
                    ...
                    if (isDisplayableError(displayableError)) {
                        ...
                        actualDisplayableError = Lang.queryJS(
                            'login.error.unknown'
                        );
                    }
    ...
    Array.from(
        ...
        val.onclick = e => {
            ...
            if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
                ...
                setOverlayContent(
                    Lang.queryJS(
                        'settings.authAccountLogout.lastAccountWarningTitle'
                    ),
                    Lang.queryJS(
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGOUT, (_, ...arguments_) => {
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            if (
                arguments_.length > 1 &&
                arguments_[1] === MSFT_ERROR.NOT_FINISHED
            ) {
                // User cancelled.
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if (res) {
                ...
                setOverlayContent(
                    Lang.queryJS('settings.dropinMods.deleteFailedTitle', {
                        fullName,
                    }),
            ...
            if (
                ...
                DropinModUtil.toggleDropinMod(
                    ...
                    if (!isOverlayVisible()) {
                        setOverlayContent(
                            Lang.queryJS(
                                'settings.dropinMods.failedToggleTitle'
                            ),
                            err.message,
                            Lang.queryJS('settings.dropinMods.okButton')
                        );
...
function populateVersionInformation(
    ...
    if (isPrerelease(version)) {
        ...
        titleElement.innerHTML = Lang.queryJS(
            'settings.about.stableReleaseTitle'
        );
        ...
        settingsUpdateButtonStatus(
            ...
            () => {
                if (!isDev) {
                    ...
                    settingsUpdateButtonStatus(
                        Lang.queryJS(
                            'settings.updates.checkingForUpdatesButton'
                        ),
                        true
                    );
                }
