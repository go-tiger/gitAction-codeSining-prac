...
const refreshServerStatus = async (fade = false) => {
    ...
    try {
        ...
        loggerLanding.warn(
            'Unable to refresh server status, assuming offline.'
        );
...
async function asyncSystemScan(effectiveJavaOptions, launchAfter = true) {
    ...
    if (jvmDetails == null) {
        ...
        setOverlayHandler(() => {
            setLaunchDetails(
                Lang.queryJS('landing.systemScan.javaDownloadPrepare')
            );
            toggleOverlay(false);
    ...
    if (login) {
        ...
        const onLoadComplete = () => {
            ...
            if (hasRPC) {
                DiscordWrapper.updateDetails(
                    Lang.queryJS('landing.discord.loading')
                );
        ...
        const gameStateChange = function (data) {
            ...
            if (SERVER_JOINED_REGEX.test(data)) {
                DiscordWrapper.updateDetails(
                    Lang.queryJS('landing.discord.joined')
                );
        ...
        try {
            ...
            if (
                ...
                proc.on('close', (code, signal) => {
                    loggerLaunchSuite.info(
                        'Shutting down Discord Rich Presence..'
                    );
